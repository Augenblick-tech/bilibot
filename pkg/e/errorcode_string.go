...
func _() {
	...
	_ = x[ERR_AUTH_FORMAT-14]
	_ = x[ERR_AUTH_IS_REFRESH_TOKEN-15]
}

const _ErrorCode_name = "读取文件错误写入文件错误创建文件错误反序列化错误序列化错误添加动态错误回复动态错误动态已存在无效数字低于阈值评论错误http状态不是200登录失败请求体中auth为空请求体中auth格式错误请求体中auth是refresh token"

var _ErrorCode_index = [...]uint16{0, 18, 36, 54, 72, 87, 105, 123, 138, 150, 162, 174, 193, 205, 227, 255, 287}
